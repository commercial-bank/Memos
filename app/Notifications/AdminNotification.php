<?php

namespace App\Notifications;

use App\Models\User;
use Illuminate\Bus\Queueable;
use Illuminate\Notifications\Notification;

class AdminNotification extends Notification
{
    use Queueable;

    public $type; 
    public $actor;  // L'admin qui fait l'action
    public $author; // L'utilisateur cible
    public $object; // <--- Nouveau : L'objet de la notification

    public function __construct(string $type, User $actor, User $author, string $object)
    {
        $this->type = $type;
        $this->actor = $actor;
        $this->author = $author;
        $this->object = $object; // Initialisation de l'objet
    }

    public function via(object $notifiable): array
    {
        return ['database']; 
    }

    public function toArray(object $notifiable): array
    {
        $message = "";
        $iconBg = "";
        $iconColor = "";
        $iconPath = "";

        switch ($this->type) {
            case 'profil':
                $message = "Profil mis à jour";
                $iconBg = 'bg-blue-100';
                $iconColor = 'text-blue-600';
                $iconPath = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>';
                break;

            case 'admin':
                $message = "Droits d'accès";
                $iconBg = 'bg-purple-100';
                $iconColor = 'text-purple-600';
                $iconPath = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path>';
                break; 

            case 'interims_delegations':
                $message = "Intérims & Délégations";
                $iconBg = 'bg-amber-100';
                $iconColor = 'text-amber-600';
                $iconPath = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path>';
                break;    
   
            default:
                $message = "Notification Système";
                $iconBg = 'bg-gray-100';
                $iconColor = 'text-gray-600';
                $iconPath = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>';
        }

        return [
            'type' => $this->type,
            'object' => $this->object, // <--- Ajouté dans le JSON stocké
            'message' => $message,
            'details' => "Action effectuée par : " . $this->actor->first_name . " " . $this->actor->last_name,
            'icon_bg' => $iconBg,
            'icon_color' => $iconColor,
            'icon_path' => $iconPath,
            'link' => '#', 
        ];
    }
}